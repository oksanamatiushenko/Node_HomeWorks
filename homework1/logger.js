...
// Откройте ваш редактор кода и создайте новую директорию для проекта,
// например, `simpleLogger`.
...
// Создайте функцию `logMessage`, которая принимает сообщение
// и записывает его в файл `log.txt`.
// Используйте метод `fs.appendFile`.
// Внутри функции используйте метод `fs.appendFile` для добавления
// нового сообщения в файл `log.txt`.
...
// `node app.js` для запуска скрипта.
...
logMessage('This is first message')
logMessage('This is second message')
